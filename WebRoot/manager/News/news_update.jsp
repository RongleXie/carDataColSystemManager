...
<body>
<jsp:include page="news!queryOne.action?id=${param.id}"></jsp:include>
...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
        ...
        <td><input type="text" name="news.promulgator"  value="${news.promulgator}"/></td>
    </tr>   
    <tr>
        <td class="tableleft">新闻类型</td>
        <td>
								<input type="radio" name="news.newstype" <c:if test="${news.newstype eq '行业新闻'}">checked</c:if>
									value="行业新闻"/>
								行业新闻
								<input type="radio" name="news.newstype"
									<c:if test="${news.newstype eq '公司新闻'}"> checked</c:if>
									value="公司新闻" />
								公司新闻
							</td>
    </tr>
    ...
    <tr>
        ...
        <td>
				<textarea name="news.content"
					style="width: 500px; height: 200px; float: left; padding: 3px 0px 0 4px; background-color: #F4F6F7; border: 1px #90A9B7 solid; color: #000; font-family: Arial, Helvetica, sans-serif; font-size: 12px; -moz-border-radius: 4px; -webkit-border-radius: 4px; border-radius: 4px;">${news.content}</textarea>
		</td>
			</tr>   
