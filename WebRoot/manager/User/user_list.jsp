...
<%
 ...
 %>
<form class="form-inline definewidth m20" action="user!searchPage.action" method="post">  
    用户名(或邮箱)：
    <input type="text" name="rolename" id="rolename"class="abc input-default" placeholder="输入用户名或邮箱查询" value="">&nbsp;&nbsp;  
    <a href="<%=basePath%>manager/User/user.jsp?flag=query"><button type="submit" class="btn btn-primary">查询</button></a>&nbsp;&nbsp; 
    <a href="<%=basePath%>manager/User/user.jsp?flag=add"><button type="button" class="btn btn-success" id="addnew">新增用户</button></a>
</form>
<table class="table table-bordered table-hover definewidth m10" >
    <thead>
    <tr>
    	<th width="10%">序号(Number)</th>
    	<th width="10%">用户名(Name)</th>
    	<th width="10%">邮箱(Email)</th>
        <th width="10%">密码(Password)</th>
        <th width="15%">总积分(TotalPoints)</th>
        <th width="15%">有效积分(ValidPoints)</th>
        <th width="15%">注册时间(Time)</th>
        <th width="20%">用户管理(Manager)</th>
    </tr>
    </thead>
			...
			<c:forEach items="${user}" var="n" varStatus="coun">
	     <tr>
            ...
            <td>${n.insertTime}</td>
            <td>
                  <a href="<%=basePath%>manager/User/user.jsp?flag=update&id=${n.userId}">编辑</a>
                  <a href="<%=basePath%>user!delete.action?id=${n.userId}"  onclick="del()">删除</a>
            </td>
    </c:forEach>
        </tr></table>
...
<div class="inline pull-right page">
        <pg:pager items="${count}" maxPageItems="10"
            export="currentNum=pageNumber" url="user!adminQueryPage.action" >
            <pg:first>
                <a href="${pageUrl}">首页</a>
            </pg:first>
            <pg:prev>
